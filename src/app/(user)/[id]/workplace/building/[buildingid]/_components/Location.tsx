    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex gap-4">
          <Floor label="전체" isSelect={selectedFloor === 'all'} onClick={() => selectFloor('all')} />
          {
            floors.map((v, i) => {
              const isSelect = selectedFloor === v;
              return <Floor key={i} label={v.name} isSelect={isSelect} onClick={() => selectFloor(v)} />
            })
          }
          <FloorAddButton />
        </div>
            ...
            floors.flatMap((floor) => {
              ...
              if (selectFloor === null || selectedFloor === 'all') {
                return floor.rooms.map((room, idx) => {
                  return <Room key={`%${idx}${room.id}`} label={room.name} floor={floor.name} />
                })
              }
              else {
                return floor === selectedFloor ? selectedFloor.rooms.map((room, idx) => {
                  return <Room key={`%${idx}${room.id}`} label={room.name} floor={floor.name} />
                }) : [];
              }
...
const Floor = ({ label, isSelect, onClick }: { label: string, isSelect: boolean, onClick: () => void }) => {

  return (
    <Card className={`px-8 py-2 rounded-sm ${isSelect ? 'bg-blue-500' : null} hover:cursor-pointer`} onClick={onClick}>
      <span className={`text-xs ${isSelect ? 'text-white' : null}`}>{label}</span>
    </Card>
  );
};

const floorFormSchema = z.object({
  name: z.string({ required_error: '필드를 입력해주세요.' })
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-8 ">
            <FormField <floorFormType>
              ...
              name="name"
              render={({ field }) => <TextFormItem label="층 명칭" placeholder="1F" field={field} />}
            />

            <Button type='submit' className="w-full bg-blue-500 hover:cursor-pointer hover:bg-blue-600">생성</Button>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-8 ">
            <FormField <roomFormType>
              control={form.control}
              name="name"
              render={({ field }) => <TextFormItem label="위치 명칭" placeholder="기계실" field={field} />}
            />
