...
import { Building2, ChevronsUpDown, Drill, FireExtinguisher, Flame, Megaphone, PaintRoller, ShieldBan, Triangle, Wifi, Zap } from "lucide-react";
...
const StatCard = ({ label, value, delta, deltaType }: { label: string, value: number, delta?: number, deltaType?: 'INCREASE' | 'DECREASE' }) => {
  return (
    <div className="flex flex-col gap-2 justify-center border-t pt-4 px-4">
      <div className="flex justify-between items-start">
        ...
        {
          delta ?
            <div className={`flex items-center gap-4 ${deltaType == 'INCREASE' ? 'bg-[#8fb38c6b]' : 'bg-red-200'} rounded-xl px-2 py-1`}>
...
const CustomToggle = ({
  ...
  return (
    ...
    >
      <div
        className={`inline-flex justify-center items-center border h-12 rounded-sm ${isPressed ? "bg-stone-400" : "bg-muted"
          } duration-300`}
      >
        {children}
      </div>
